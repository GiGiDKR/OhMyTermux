...
# OhMyTermux installation
ohmytermux() {
    $HOME/.config/OhMyTermux/install.sh "$@"
}
...
EOL" "Termux.properties configuration"
    ...
    execute_command "curl -fLo \"$HOME/.config/OhMyTermux/help.md\" https://raw.githubusercontent.com/GiGiDKR/OhMyTermux/refs/heads/1.0.0/src/help.md" "Download of the help file"
}
...
install_prompt() {
    ...
    subtitle_msg "❯ Prompt installation"
    ...
    if $PROOT_CHOICE; then
        ...
        if [ -n "$PROOT_USERNAME" ] && [ -n "$PROOT_PASSWORD" ]; then
            ...
            if $USE_GUM; then
                ...
                if [[ "$CHOICE" =~ ^[oO]$ ]]; then
                    execute_command "pkg install proot-distro -y" "Installation of proot-distro"
                    download_and_execute "https://raw.githubusercontent.com/GiGiDKR/OhMyTermux/1.0.0/proot.sh" "PRoot"
                    install_utils
                fi
            fi
        fi
...
install_termux_x11() {
    if $X11_CHOICE; then
        title_msg "❯ Termux-X11 installation"
